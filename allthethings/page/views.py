...
def get_additional_for_aarecord(aarecord):
    ...
    additional['table_row'] = {
        ...
        'id_name': "".join([ # Note, not actually necessary to join, should be mutually exclusive.
            ...
            gettext('page.md5.toprow.edsebk', id=aarecord_id_split[1]).replace('}','') if aarecord_id_split[0] == 'edsebk' else '',
            gettext('page.md5.toprow.cerlalc', id=aarecord_id_split[1]).replace('}','') if aarecord_id_split[0] == 'cerlalc' else '',
            gettext('page.md5.toprow.czech_oo42hcks', id=aarecord_id_split[1]).replace('}','') if aarecord_id_split[0] == 'czech_oo42hcks' else '',
            gettext('page.md5.toprow.gbooks', id=aarecord_id_split[1]).replace('}','') if aarecord_id_split[0] == 'gbooks' else '',
            gettext('page.md5.toprow.goodreads', id=aarecord_id_split[1]).replace('}','') if aarecord_id_split[0] == 'goodreads' else '',
            gettext('page.md5.toprow.isbngrp', id=aarecord_id_split[1]).replace('}','') if aarecord_id_split[0] == 'isbngrp' else '',
            gettext('page.md5.toprow.libby', id=aarecord_id_split[1]).replace('}','') if aarecord_id_split[0] == 'libby' else '',
            gettext('page.md5.toprow.rgb', id=aarecord_id_split[1]).replace('}','') if aarecord_id_split[0] == 'rgb' else '',
            gettext('page.md5.toprow.trantor', id=aarecord_id_split[1]).replace('}','') if aarecord_id_split[0] == 'trantor' else '',
        ]),
